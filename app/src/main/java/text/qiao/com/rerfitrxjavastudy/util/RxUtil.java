...
import text.qiao.com.rerfitrxjavastudy.model.net.response.GankHttpResponse;
import text.qiao.com.rerfitrxjavastudy.model.net.response.GoldHttpResponse;
import text.qiao.com.rerfitrxjavastudy.model.net.response.MyHttpResponse;
import text.qiao.com.rerfitrxjavastudy.model.net.response.WXHttpResponse;
...
public class RxUtil {
     ...
     * @return
     */
    public static <T> FlowableTransformer<T, T> stringResult() {   //compose判断结果
        return new FlowableTransformer<T, T>() {
            @Override
            public Flowable<T> apply(Flowable<T> httpResponseFlowable) {
                return httpResponseFlowable.flatMap(new Function<T, Flowable<T>>() {
                    ...
                    public Flowable<T> apply(T tHttpResponse) {
                        ...
                        return Flowable.error(new ApiException("当前网络不可用，请检查网络！"));
                    }
                });
            }
        };
